...
//  MyPlaces
//
//  Created by Rinat Abidullin on 15.02.2020.
    ...
    private func getDirections() {
        ...
        directions.calculate { (response, error) in
            ...
            for route in response.routes {
                ...
                let timeImterval = route.expectedTravelTime
                
                print("Расстояние до места: \(distance) км.")
                print("Время в пути: \(timeImterval) сек.")
            }
        }
    }
...
extension MapViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, regionDidChangeAnimated animated: Bool) {
        ...
        geocoder.reverseGeocodeLocation(center) { (placemarks, error) in
            ...
            let buidingNumber = placemark?.subThoroughfare
            
            DispatchQueue.main.async {
                if streetName != nil && buidingNumber != nil {
                    self.addressLabel.text = "\(streetName!), \(buidingNumber!)"
